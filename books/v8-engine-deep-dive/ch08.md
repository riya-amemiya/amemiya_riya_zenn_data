---
title: "第7章 Number, HeapNumber, BigInt, Oddball"
free: true
---
...
数値とプリミティブの表現を解剖しました。第 8 章までで V8 の値表現はすべて見終えました。次章ではメモリ管理に踏み込み、ポインタ圧縮の 4GB Cage、複数 Compression Scheme (Main / Trusted / ExternalCode)、V8 Sandbox による分離、Indirect Pointer、Code Pointer Table、JS Dispatch Table を扱います。
