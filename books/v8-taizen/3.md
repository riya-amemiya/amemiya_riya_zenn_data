...
# V8 の配列を深く理解するための完全解説

## 本章の位置づけ

本章は V8 (main ブランチを土台にしています) の Array 関連実装について、ElementsKind の型システムから始まり、メモリレイアウト、配列リテラルの初期化、各種配列メソッドの内部実装、最適化コンパイラによるインライン化、Protector cells による楽観的最適化、Garbage Collection との関係、ティアアップによる動的最適化、SIMD 命令の活用まで、ソースコードの該当箇所を引用しながら全方位に解説するものです。
...
|----|--------|
| 第1章 | V8 における配列とは何か |
| 第2章 | ElementsKind ― V8 の配列型システム |
| 第3章 | メモリレイアウト ― タグ付きポインタとヒープ配置 |
| 第4章 | 値の格納と表現 ― Smi と HoleNaN と辞書 |
| 第5章 | 容量の拡大と縮小 ― バッキングストア管理 |
| 第6章 | Torque ― V8 独自のビルトイン言語 |
| 第7章 | 配列メソッドの V8 内部実装 |
...
| 第14章 | SIMD 最適化 ― indexOf と includes |
