...
JS大改革のES6に実は入ってるメソッドなんですよね。`flat` や `includes` のようなメジャーなメソッドと比べると、影の薄いメソッドですが、配列の一部を配列内の他の場所にシャローコピーする処理を高速に行えるメソッドです。
...
d8（arm64.release）で計測しました。1M要素の配列に対し、重なりのない `copyWithin(0, N/2)` と重なりのある `copyWithin(1, 0)` を、ウォームアップ後にbest-of-8で計測しています。
`before` がfast path導入前のmain、`after` が導入後です。

pointer compressionを有効にしたビルドでは、次のようになります。

| ケース | before | after | 速度比 |
| --- | --- | --- | --- |
| packed smi | 10.55 ms/op | 0.0243 ms/op | 約434x |
| packed double | 11.06 ms/op | 0.0491 ms/op | 約225x |
| packed object | 10.68 ms/op | 0.0251 ms/op | 約426x |
| holey smi | 11.76 ms/op | 0.0252 ms/op | 約467x |
| holey double | 11.49 ms/op | 0.0491 ms/op | 約234x |
| packed smi (overlap) | 21.83 ms/op | 0.0617 ms/op | 約354x |
| packed double (overlap) | 22.68 ms/op | 0.1225 ms/op | 約185x |

pointer compressionを無効にしたビルドでは、次のようになります。

| ケース | before | after | 速度比 |
| --- | --- | --- | --- |
| packed smi | 9.25 ms/op | 0.0470 ms/op | 約197x |
| packed double | 10.67 ms/op | 0.0480 ms/op | 約222x |
| packed object | 9.38 ms/op | 0.0501 ms/op | 約187x |
| holey smi | 9.47 ms/op | 0.0483 ms/op | 約196x |
| holey double | 11.06 ms/op | 0.0496 ms/op | 約223x |
| packed smi (overlap) | 18.50 ms/op | 0.1224 ms/op | 約151x |
| packed double (overlap) | 21.30 ms/op | 0.1219 ms/op | 約175x |

pointer compressionが有効だと `FixedArray` の1要素が4バイトになるため、smiとobjectのfast pathが動かすバイト数は半分になります。
`FixedDoubleArray` は圧縮の対象外で常に8バイトなので、doubleの速度比は圧縮の有無でほとんど変わりません。

同じ計測コードをJavaScriptCore（WebKit trunk、4477de733d5b）でも走らせ、最適化後の `after` と比べました。

| ケース | JSC | V8（圧縮有効） | 速度比 | V8（圧縮無効） | 速度比 |
| --- | --- | --- | --- | --- | --- |
| packed smi | 0.1587 ms/op | 0.0243 ms/op | 約6.5x | 0.0470 ms/op | 約3.4x |
| packed double | 0.1440 ms/op | 0.0491 ms/op | 約2.9x | 0.0480 ms/op | 約3.0x |
| packed object | 0.1680 ms/op | 0.0251 ms/op | 約6.7x | 0.0501 ms/op | 約3.4x |
| holey smi | 0.1627 ms/op | 0.0252 ms/op | 約6.5x | 0.0483 ms/op | 約3.4x |
| holey double | 0.1467 ms/op | 0.0491 ms/op | 約3.0x | 0.0496 ms/op | 約3.0x |
| packed smi (overlap) | 0.2360 ms/op | 0.0617 ms/op | 約3.8x | 0.1224 ms/op | 約1.9x |
| packed double (overlap) | 0.2187 ms/op | 0.1225 ms/op | 約1.8x | 0.1219 ms/op | 約1.8x |

JSCは値表現がsmiとdoubleで同じ幅なので、圧縮無効のV8と比べたほうが動かすバイト数が揃っており、より厳密な比較と思います。
