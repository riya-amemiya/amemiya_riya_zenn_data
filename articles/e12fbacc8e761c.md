...
メモリ上では、サブ配列のbacking storeから結果配列の該当位置へ、ブロックをそのまま流し込むイメージです。

```mermaid
flowchart LR
    subgraph src ["サブ配列 [1, 2, 3] の backing store"]
        direction LR
        S0["1"] --- S1["2"] --- S2["3"]
    end

    subgraph dst ["結果配列の backing store"]
        direction LR
        D0["書込済"] --- D1["1"] --- D2["2"] --- D3["3"] --- D4["未書込"]
    end

    src ==>|memcpy で srcLen 個を一括コピー| dst
```
...
`[[1, 2, 3], [4, 5, 6]].flat()` を例に、前回と今回の2つのパッチで処理がどう変わったのかを並べてみます。

```mermaid
flowchart TB
    subgraph v0 ["① 最適化前のV8"]
        direction TB
        A0["空の結果配列を作成"] --> A1["[1,2,3]を発見 → IsArray → 再帰"]
        A1 --> A2["1を追加 → 容量チェック"]
        A2 --> A3["2を追加 → 容量チェック"]
        A3 --> A4["3を追加 → 再割り当ての可能性"]
        A4 --> A5["[4,5,6]を発見 → IsArray → 再帰"]
        A5 --> A6["4, 5, 6を追加 → さらに再割り当て"]
        A6 --> A7["途中でElementsKind遷移が起き得る"]
    end

    v0 ~~~ v1

    subgraph v1 ["② 2パス方式（前回のパッチ）"]
        direction TB
        B0["第1パス: 外側2要素を走査<br>各サブ配列の.lengthを加算 → 長さ6, kind=SMI"] --> B1["長さ6で1回だけ確保"]
        B1 --> B2["第2パス: 6要素を1つずつ直接書き込み<br>再割り当てなし / 遷移なし"]
    end

    v1 ~~~ v2

    subgraph v2 ["③ バルクコピー（今回のパッチ）"]
        direction TB
        C0["第1パス: 長さ6を確定"] --> C1["1回だけ確保<br>doubleならhole埋めmemsetも省略"]
        C1 --> C2["第2パス: [1,2,3]をmemcpy<br>[4,5,6]をmemcpy"]
        C2 --> C3["要素ごとのチェックがコピーから消える"]
    end
```

最適化前のV8では、空配列から始めて要素を1つずつ追加し、容量が足りなくなるたびに再割り当てが走っていました。
サブ配列ごとに再帰呼び出しも発生し、追加する値の型によってはElementsKindの遷移と全要素のコピーまで起きていました。

2パス方式では、先に長さを数えて1回だけ確保するため、再割り当てとElementsKindの遷移が消えました。
ただし第2パスのコピーは1要素ずつのままで、要素ごとのチェックが残っていました。

今回のバルクコピーでは、サブ配列が数値配列なら要素ごとの読み書きそのものをやめ、backing storeを `memcpy` で一気に移します。
これで、純粋なコピー以外の付帯処理がコピー本体から消えます。

段階ごとに、どの処理が削れたのかを積み上げると次のようになります。

```mermaid
flowchart LR
    R0["最適化前のV8"] -->|2パス方式| R1["再割り当て: 複数回 → 0<br>ElementsKind遷移: あり → 0"]
    R1 -->|バルクコピー| R2["要素ごとの読み書き → memcpyで一括<br>hole埋めmemset: あり → 省略"]
    R2 --> R3["合計で約20倍の速度"]
```
