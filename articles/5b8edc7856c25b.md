...
flowchart LR
    subgraph asIs ["旧実装"]
        ...
        A2 --> A3{"index < end？"}
        A3 -- Yes --> A4["SetImpl で1要素書く<br>値の変換・分岐・write barrier"]
        A4 --> A5["index を1つ進める"]
        A5 --> A3
        A3 -- No --> A6["配列を返す"]
    end
    ...
    subgraph toBe ["新実装"]
        direction TB
        B1["容量を確保"] --> B2{"ElementsKind"}
        B2 -- "FixedArray かつ値は Smi" --> B3["MemsetTagged 1回"]
        B2 -- "FixedArray かつ値が Smi 以外" --> B4["SetImpl ループ<br>オブジェクトなど"]
        B2 -- "FixedDoubleArray" --> B5["値を1回 double 化して密に書く"]
        B3 --> B6["配列を返す"]
        B4 --> B6
        B5 --> B6
    end
