...
![](/images/5b8edc7856c25b/range.gif)

`start` と `end` は省略でき、その場合は全てのindexを埋めます

また、 `fill` は破壊的に書き換えて同じ配列を返します。
新しい配列は作りません。`length` も変わりません。
...
`TryFastArrayFill` はElementsKindの遷移を済ませたあと、最終的に `ElementsAccessor::Fill` 経由で `FillImpl` を呼びます。今回のパッチが触ったのは、この `FillImpl` です。
...
![](/images/5b8edc7856c25b/paint.gif)
...
![](/images/5b8edc7856c25b/convert.gif)
...
| ケース | before | after | 倍率 |
| --- | ---: | ---: | ---: |
| `smi_fill_full` | 34.67 us/op | 5.07 us/op | 約6.8x |
| `smi_fill_partial` | 30.46 us/op | 4.53 us/op | 約6.7x |
| `holey_smi_fill_full` | 36.46 us/op | 4.96 us/op | 約7.4x |
| `double_fill_full` | 51.37 us/op | 10.10 us/op | 約5.1x |
| `double_fill_partial` | 45.85 us/op | 9.01 us/op | 約5.1x |
| `double_fill_1M` | 514.6 us/op | 99.9 us/op | 約5.2x |
| `smi_fill_small_n16` | 0.020 us/op | 0.015 us/op | 約1.3x |
| `object_fill_full` | 55.6 us/op | 55.6 us/op | 約1.0x |
