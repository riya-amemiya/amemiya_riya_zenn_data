...
数値だけの配列はサブ配列を含まないので、深さがどれだけ残っていても、それ以上は平坦化されません。
第1パスも同じ理由で、深さに関係なく数値サブ配列の `.length` をそのまま足しています。
第2パスの限定を外すと、この第1パスの数え方と一致します。
おかげで `[[1, 2, 3]].flat(5)` のように深さ指定が大きくても、一括コピーが効きます。

## ベンチマーク

d8（arm64）で、最適化前のV8（14.6.206）と最適化後（main）を比べました。
ここでの最適化前は2つのパッチが入る前、最適化後は両方が入った状態です。
合計100万要素（1000要素のサブ配列が1000個）の配列を `flat(2)` で平坦化し、ウォームアップ後に8回計測した最小値です。
...
大きく伸びたのは、数値だけのPacked配列です。
`PACKED_SMI_ELEMENTS` で約24倍、`PACKED_DOUBLE_ELEMENTS` で約14倍になりました。
一括コピーの対象外である残りの型も、2パス方式の1回確保で1.5倍ほど速くなっています。
Packed数値配列だけが飛び抜けるのは、ここにバルクコピーが効いているからです。

効果はサブ配列が大きいほど伸びます。
1000要素のサブ配列なら、1000回の個別書き込みが1回のコピーに置き換わるからです。

:::message
計測環境やマシンのウォームアップ状態によって誤差があります。
あくまで個人のPCで計測した一例です。
:::
