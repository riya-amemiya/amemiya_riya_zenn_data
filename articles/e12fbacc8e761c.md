...
コードはあくまで補足とし、詳しく見たい方向けに折りたたみで添えるだけにとどめます。
...
そこで今回は、サブ配列が「数値だけのPacked配列」のとき、backing storeをまるごとコピーするショートカットを足しました。

V8には `TorqueCopyElements` という部品があり、その中身は最終的に `libc` の `memcpy` を呼びます。
これはメモリのブロックを一気に複製する操作で、CPUが連続コピーに最適化されているぶん、1個ずつ書くより圧倒的に速いです。

イメージとしては、サブ配列のメモリ領域から、結果配列の書き込み位置へ、塊をそのまま流し込む感じです。
...
:::details V8に詳しい方向け（実際のTorqueコード）
Smiサブ配列に対する最終的な実装はこうなっています。
`subArray.length` 分のブロックを `TorqueCopyElements` で一括コピーし、`targetIndex` をまとめて進めています。
...
身近なたとえでいうと、書庫の書類が机のメモを指しているとき、その対応を専用の付箋（記録）に控えておくようなものです。
付箋を更新せずにメモだけ動かすと、GCはメモがもう使われていないと勘違いして、まだ参照されているメモを処分してしまいます。
