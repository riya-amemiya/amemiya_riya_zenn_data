...
1. 最もシンプルな初期化
   - 小規模なスクリプトや、一時的な使用に適しています
   - 特別な設定は必要ありません

2. コンパイラオプションを指定
   - 特定のTypeScriptの機能に依存する場合に使用します
   - たとえば、デコレータを使用する場合などに必要です

3. tsconfig.jsonを使用（推奨）
   - 既存のTypeScriptプロジェクトで使用する場合に最適です
   - プロジェクトの設定と整合性が取れます
...
ts-morphの強力な機能の一つが、コードの安全な修正です。
...
私はts-morphを使って、先日1200ファイル以上のimport文を一括修正する作業を行いました。手動でやると丸一日かけても終わらない作業を、ts-morphを使うことで数時間で完了させることができました。
