...
flowchart LR
    ...
    subgraph toBe ["新実装"]
        direction TB
        B1{"ElementsKind"} -- "double" --> B2{"探索値"}
        B1 -- "Smi / object" --> B5["TaggedEqual を先に見る<br>数値は float64、他は StrictEqual"]
        B2 -- "NaN 以外の数値" --> B3["生の float64 同士で比較"]
        B2 -- "NaN や非数値" --> B4["走査せず -1"]
        B2 -- "undefined" --> B6["センチネルだけ走査"]
        B3 --> B7["index か -1"]
        B5 --> B7
        B6 --> B7
    end
