...
全て `number` 型です。
JavaScriptには数値の判定系メソッドがたくさんありますが、その中で、`isFinite`と`Number.isFinite` は `float` 型を判定することができます。
...
float型は有理数かつ、整数でない数値となるため、`Number.isFinite` と `Number.isInteger` を使って判定することができます。
