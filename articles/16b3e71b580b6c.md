...
この記事のHTMLバージョンです。CSSの方がかなり評判良く、ありがたいことに多くの方に読んでいただいています。

https://zenn.dev/gmomedia/articles/6ed3718b4c6bc1

私は自称フロントエンドエンジニアなので、せっかくならHTMLの方もちゃんと学んでみようと思い、最近のHTMLの新機能を改めて学び直したので、アウトプットついでにこの記事を書いています。
...
`popovertarget`属性は、ボタン要素と対象ポップオーバーの関係を HTML マークアップレベルで宣言します。この関係性により、ブラウザは適切なイベントハンドリングとアクセシビリティ属性を自動的に設定します。
...
`dialog`要素の最も強力な機能の一つは、`form`要素との深い統合です。`method="dialog"`を指定したフォームは、送信時に自動的にダイアログを閉じ、ボタンの`value`属性の値を`dialog.returnValue`に設定します。
...
`details`要素は HTML5 から存在していましたが、複数の `details` 要素が独立して動作するため、一般的なアコーディオン UI（一度に一つだけ開く）を実現するには `JavaScript` による状態管理が必要でした。
...
`name`属性による排他制御は、DOM イベントレベルで実装されています。`details`要素の `open` 属性が変更される際、ブラウザは同じ `name` 値を持つ他の `details` 要素の `open` 属性を自動的に削除します。

## 4. inert属性による包括的無効化

### HTML仕様レベルでの無効化定義

`inert`属性は、指定した要素とその子要素を完全に無効化する HTML 属性です。無効化の範囲は、フォーカス可能性、イベント応答性、アクセシビリティツリーからの除外など、ブラウザの要素処理全般に及びます。
...
従来の `disabled` 属性がフォーム要素にのみ適用可能だったのに対し、`inert`属性はあらゆる HTML 要素に適用できます。
...
```html
<!-- クリティカルなスタイル：レンダリングをブロック -->
<link rel="stylesheet"
      href="critical-styles.css"
      blocking="render">

<!-- 非クリティカルなスタイル：レンダリングをブロックしない -->
<link rel="stylesheet"
      href="animations.css">

<!-- 重要なスクリプト：レンダリングをブロック -->
...
2019年から2025年にかけて、HTMLは要素間の関係性を宣言することで複雑なUI動作を実現する言語へと進化しました。
