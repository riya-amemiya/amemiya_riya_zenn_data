...
本章は V8 の基底層 (Tagged Pointer、HeapObject、Map) から最上層 (Inline Cache、TurboFan、Code オブジェクト) までを 1 本の縦串で解剖します。後続章で展開するメモリ管理、配列、Torque、ケーススタディの全てが、本章で導入する語彙と不変条件に依拠します。

## 本章で扱う 12 のレイヤ

| 節 | テーマ | 中核データ構造 |
|----|--------|----------------|
| 第1章 | Tagged Pointer と Smi | `Tagged<T>`, `Smi`, `MaybeObject` |
| 第2章 | HeapObject と Object 階層 | `HeapObject`, `MapWord`, `Handle` |
| 第3章 | Map と Transition Tree | `Map`, `DescriptorArray`, `TransitionArray` |
| 第4章 | JSObject の Properties / Elements | `JSObject`, `PropertyArray`, `SwissNameDictionary` |
| 第5章 | ElementsKind 完全列挙と遷移 | `ElementsKind`, `FixedArray`, `FixedDoubleArray` |
| 第6章 | String 階層 | `SeqString`, `ConsString`, `SlicedString`, `ExternalString`, `ThinString` |
| 第7章 | Number, HeapNumber, BigInt, Oddball | `HeapNumber`, `BigInt`, `Oddball`, `Hole` |
| 第8章 | Pointer Compression Cage と V8 Sandbox | `Tagged_t`, `Isolate::cage_base`, External Pointer Table |
| 第9章 | Heap Spaces とメモリレイアウト | `NewSpace`, `OldSpace`, `LargeObjectSpace`, `CodeRange` |
| 第10章 | Orinoco GC | Scavenger, Mark-Compact, Concurrent Marking |
| 第11章 | 実行パイプライン | Ignition, Sparkplug, Maglev, TurboFan, Turboshaft |
| 第12章 | Inline Cache と FeedbackVector | `FeedbackVector`, `LoadHandler`, `StoreHandler` |
