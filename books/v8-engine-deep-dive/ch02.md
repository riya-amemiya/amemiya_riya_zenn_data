---
title: "第1章 Tagged Pointer と Smi - V8 における値の最小単位"
free: true
---

## 第 I 部 V8 基礎の位置づけ

本章から始まる 12 章 (ch02 - ch13) が第 I 部 V8 基礎です。V8 の値表現の最小単位である Tagged Pointer から、最上位の Inline Cache と TurboFan までを、ひとつの語彙体系で 1 周します。各章は次のように積み上がります。

| 章 | テーマ | 中核データ構造 |
|----|--------|----------------|
| 第 1 章 (本章) | Tagged Pointer と Smi | `Tagged<T>`, `Smi`, `MaybeObject` |
| 第 2 章 | HeapObject と Object 階層 | `HeapObject`, `MapWord`, `Handle` |
| 第 3 章 | Map (Hidden Class) と Transition Tree | `Map`, `DescriptorArray`, `TransitionArray` |
| 第 4 章 | JSObject のレイアウトと Properties / Elements | `JSObject`, `PropertyArray`, `SwissNameDictionary` |
| 第 5 章 | ElementsKind 完全列挙と遷移 | `ElementsKind`, `FixedArray`, `FixedDoubleArray` |
| 第 6 章 | String 階層 | `SeqString`, `ConsString`, `SlicedString`, `ExternalString`, `ThinString` |
| 第 7 章 | Number, HeapNumber, BigInt, Oddball | `HeapNumber`, `BigInt`, `Oddball`, `Hole` |
| 第 8 章 | Pointer Compression Cage と V8 Sandbox | `Tagged_t`, `Isolate::cage_base`, External Pointer Table |
| 第 9 章 | Heap Spaces とメモリレイアウト | `NewSpace`, `OldSpace`, `LargeObjectSpace`, `CodeRange` |
| 第 10 章 | Orinoco GC | Scavenger, Mark-Compact, Concurrent Marking |
| 第 11 章 | 実行パイプライン | Ignition, Sparkplug, Maglev, TurboFan, Turboshaft |
| 第 12 章 | Inline Cache と FeedbackVector | `FeedbackVector`, `LoadHandler`, `StoreHandler` |

本章では V8 が JavaScript の値を「ポインタサイズの整数」として統一的に表現するため、ポインタの最下位ビット (LSB) にタグを埋め込む設計を解剖します。`kHeapObjectTag = 1` と `kSmiTag = 0` という配置がもたらす整数演算の効率、Strong / Weak / Cleared / Forwarding の 4 状態のビットレイアウト、Smi 範囲の 31/32 ビット切り替え、`Tagged<T>` テンプレートの型階層、`is_subtype_v` による静的検査までを、`/home/user/v8` のソースを行番号付きで引きながら扱います。
...
V8 の値表現の最小単位 — Tagged Pointer と Smi — を解剖しました。Tagged Pointer の最下位ビットが指す先 (タグが 1 のとき) はヒープ上のオブジェクトです。次章では、その指し先である HeapObject の構造と、HeapObject の先頭バイトが必ず Map ポインタになっているという V8 の最も基本的な不変条件を扱います。
